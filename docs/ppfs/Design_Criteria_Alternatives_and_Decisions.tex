...
The only mesh network library found was for a radio frequency (RF) radio, though that may not a disadvantage as it may be more power efficient than some other solution. Also, a lot of libraries use code that does not take power efficiency into account, which limits either the battery life of the libraries available.
...
The TI SensorTag is a small board made for prototyping IoT devices and is shown in Figures \ref{fig:TISensorTag1} and \ref{fig:TISensorTag2}. 
...
However, the devpack comes various accessories including with a 1.3"ù memory LCD, while the only desired component is the connector for memory LCD. The larger memory LCD display would have to be bought separately. The price of a display interface with this board would likely be about \$70. This cost could be further reduced if the breakout board for the memory LCD display can be replicated so that the smartwatch devpack would not need to be bought for each display interface.
